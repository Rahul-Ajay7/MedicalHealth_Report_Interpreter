...
from app.services.llm_chat import MEDICAL_DISCLAIMER_EN
...
async def get_full_report(report_id: str, request: Request, user=Depends(verify_token)):
    ...
    # ── Stored bilingual PDF disclaimer (best-effort; column may be absent) ──
    # Prefer the verbatim text saved at analyze time; fall back to English.
    # Queried separately + guarded so a missing column never breaks the fetch.
    disclaimer = MEDICAL_DISCLAIMER_EN
    try:
        dr = supabase.from_("analysis").select("medical_disclaimer") \
            .eq("report_id", report_id).eq("user_id", user_id).limit(1).execute()
        if dr.data and dr.data[0].get("medical_disclaimer"):
            disclaimer = dr.data[0]["medical_disclaimer"]
    except Exception:
        pass
