...
# Fixed medical disclaimer for the downloadable PDF. English is always shown;
# the report's chosen language is appended below it (bilingual) when known.
...
def _bilingual_disclaimer(language: str | None) -> str:
    """English disclaimer, plus a translation below it when a non-English
    language is known. Degrades to English-only on any failure."""
...
async def get_full_report(report_id: str, request: Request, user=Depends(verify_token)):
    ...
    # ── Output language (best-effort; column may not exist on older schemas) ──
    language = None
    try:
        lr = supabase.from_("analysis").select("language") \
            .eq("report_id", report_id).eq("user_id", user_id).limit(1).execute()
        if lr.data:
            language = lr.data[0].get("language")
    except Exception:
        language = None
    ...
    return {
        ...
        "medical_disclaimer": _bilingual_disclaimer(language),
    }